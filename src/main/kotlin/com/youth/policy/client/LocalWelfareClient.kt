...
open class LocalWelfareClient(
    @Value("\${local.api.list.url}")   private val listApiUrl: String,
    @Value("\${local.api.detail.url}") private val detailApiUrl: String,
    @Value("\${local.api.key}")        private val serviceKey: String
) {
    ...
    open fun getLocalWelfareList(request: LocalWelfareListRequest): LocalWelfareListResponse {
        val uri = UriComponentsBuilder
            ...
            .queryParam("serviceKey", URLEncoder.encode(serviceKey, StandardCharsets.UTF_8.name()))
